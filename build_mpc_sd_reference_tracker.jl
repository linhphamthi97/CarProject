...
function build_mpc(horizon_length)
    ...
    #         x[1,i] == x[1, i-1] + (x[4, i-1] * cos(x[3, i-1]+C1 * u[2, i-1])/(1- curvature(x[1, i-1], x[2, i-1]) *x[2, i-1]))*delta_t)
    # @NLconstraint(model, [i=2:horizon_length], x[2,i] == x[2, i-1] + (x[4, i-1] * sin(x[3, i-1]+C1 * u[2, i-1]))*delta_t)
    # @NLconstraint(model, [i=2:horizon_length], x[3,i] == x[3, i-1] + (C2 * u[2, i-1] -
    #         (x[4, i-1] * cos(x[3, i-1]+C1 * u[2, i-1])*curvature(x[1, i-1], x[2, i-1]))/(1- curvature(x[1, i-1], x[2, i-1]) *x[2, i-1]))*delta_t)
    # @NLconstraint(model, [i=2:horizon_length],
    # x[4,i] == x[4, i-1] + (Cm1 * u[1,i-1] - Cm2 * u[1,i-1] * x[4,i-1] - Cr2 * x[4, i-1] * x[4, i-1] - Cr1)* delta_t)

    # @NLconstraint(model, [i=2:horizon_length],
    #         x[1,i] == x[1, i-1] + (x[4, i-1] * cos(x[3, i-1]+atan(0.5*tan(C1*u[2, i-1])))/(1- curvature(x[1, i-1], x[2, i-1]) *x[2, i-1]))*delta_t)
    # @NLconstraint(model, [i=2:horizon_length], x[2,i] == x[2, i-1] + (x[4, i-1] * sin(x[3, i-1]+atan(0.5*tan(C1*u[2, i-1]))))*delta_t)
    # @NLconstraint(model, [i=2:horizon_length], x[3,i] == x[3, i-1] + (x[4,i-1]*sin(atan(0.5*tan(C1*u[2, i-1])))/l_r -
    # (x[4, i-1] * cos(x[3, i-1]+atan(0.5*tan(C1*u[2, i-1]))) * curvature(x[1, i-1], x[2, i-1]))/
    # (1- curvature(x[1, i-1], x[2, i-1]) *x[2, i-1]))*delta_t)
     # @NLconstraint(model, [i=2:horizon_length],
     # x[4,i] == x[4, i-1] + (Cm1 * u[1,i-1] - Cm2 * u[1,i-1] * x[4,i-1] - Cr2 * x[4, i-1] * x[4, i-1] - Cr1)* delta_t)


    # @NLconstraint(model, [i=2:horizon_length],
    #         x[1,i] == x[1, i-1] + (x[4, i-1] * cos(x[3, i-1])/(1- 0.5 *x[2, i-1]))*delta_t)
    @NLconstraint(model, [i=2:horizon_length],
            x[1,i] == x[1, i-1] + (x[4, i-1] * cos(x[3, i-1])/(1- curvature_substitute(x[1, i-1]) *x[2, i-1]))*delta_t)

    @NLconstraint(model, [i=2:horizon_length], x[2,i] == x[2, i-1] + (x[4, i-1] * sin(x[3, i-1]))*delta_t)

    # @NLconstraint(model, [i=2:horizon_length], x[3,i] == x[3, i-1] + (x[4,i-1]*sin(atan(0.5*tan(C1*u[2, i-1])))/l_r -
    #  (x[4, i-1] * cos(x[3, i-1]) * 0.5)/(1- 0.5 *x[2, i-1]))*delta_t)
    @NLconstraint(model, [i=2:horizon_length], x[3,i] == x[3, i-1] + (x[4,i-1]*sin(atan(0.5*tan(C1*u[2, i-1])))/l_r -
     (x[4, i-1] * cos(x[3, i-1]) * curvature_substitute(x[1, i-1]))/
     (1- curvature_substitute(x[1, i-1]) *x[2, i-1]))*delta_t)

    # @NLconstraint(model, [i=2:horizon_length], x[4,i] == x[4, i-1] + u[1, i-1]* delta_t)
